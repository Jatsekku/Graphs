...
class GraphAdjMx{
  public:
	...
	void dijkstra(Vertex* s)
	{
		...
		priority_queue<pq_node_t<int>> Q(vertices_amount);

		for(auto v = vertex_list.begin(); v != vertex_list.end(); v++) //for all v e G.vertices
		{
			map[V_idx] = *v;
			if(*v == s)
				distance[V_idx] = 0;				//SetDistance(v, 0)
			else
				distance[V_idx] = 2147483647;		//SetDistance(v, inf)
		}
		//setlocator
		while(!Q.empty())
		{
			pq_node_t<int> u = Q.removeMin();
			for(int e = 0; e <= vertices_amount; e++)	//for all e e G.indicentEdges(u)
			{
				Edge** e_val = incidentEdges(map[u.data]);
				Vertex* z = opposite(map[u.data], e_val[e]);
				int r = distance[u.data] + e_val[e].weight;
				if(r < distance[z->idx])
					distance[z->idx] = r;
			}
		}
